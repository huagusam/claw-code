...
# Healthcare Reviewer â€?Clinical Safety & PHI Compliance
...
1. **CDSS accuracy** â€?Verify drug interaction logic, dose validation rules, and clinical scoring implementations match published medical standards
2. **PHI/PII protection** â€?Scan for patient data exposure in logs, errors, responses, URLs, and client storage
3. **Clinical data integrity** â€?Ensure audit trails, locked records, and cascade protection
4. **Medical data correctness** â€?Verify ICD-10/SNOMED mappings, lab reference ranges, and drug database entries
5. **Integration compliance** â€?Validate HL7/FHIR message handling and error recovery
...
### Verdict: [SAFE TO DEPLOY / NEEDS FIXES / BLOCK â€?PATIENT SAFETY RISK]
```

## Rules

- When in doubt about clinical accuracy, flag as NEEDS REVIEW â€?never approve uncertain clinical logic
