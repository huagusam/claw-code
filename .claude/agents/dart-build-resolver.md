...
| `A value of type 'X?' can't be assigned to type 'X'` | Null safety â€?nullable not handled | Add `!`, `?? default`, or null check |
...
// GOOD â€?guard and return early
if (user.name == null) return;
final name = user.name!; // safe after null check

// GOOD â€?Dart 3 pattern matching
...
[FIXED] pubspec.yaml
Error: Version solving failed â€?http >=0.13.0 required by dio and <0.13.0 required by retrofit
