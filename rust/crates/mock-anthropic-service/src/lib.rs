...
fn build_stream_body(request: &MessageRequest, scenario: Scenario) -> String {
    match scenario {
        ...
        Scenario::WriteFileDenied => match latest_tool_result(request) {
            Some((tool_output, _)) => {
                final_text_sse(&format!("new_file denied as expected: {tool_output}"))
            }
            None => tool_use_sse(
                "toolu_write_denied",
                "new_file",
    ...
    match scenario {
        ...
        Scenario::WriteFileAllowed => match latest_tool_result(request) {
            Some((tool_output, _)) => text_message_response(
                ...
                &format!("new_file succeeded: {}", extract_file_path(&tool_output)),
            ),
            None => tool_message_response(
        ...
        Scenario::WriteFileDenied => match latest_tool_result(request) {
            Some((tool_output, _)) => text_message_response(
                ...
                &format!("new_file denied as expected: {tool_output}"),
            ),
            None => tool_message_response(
