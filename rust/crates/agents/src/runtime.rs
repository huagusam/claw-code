...
impl ApiClient for ProviderRuntimeClient {
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        // Compute a tools fingerprint to detect changes across requests.
        // Used for both skip_tools (Anthropic-only) and cache persistence.
        ...
        for (index, entry) in chain.iter().enumerate() {
            // Provider-specific tool delivery strategy:
            //
            // Anthropic — server-side prompt caching via `cache_control: ephemeral`.
            //   requests 2+ can skip sending tools entirely (skip_tools=true).
            //
            // OpenAI-compat (cloud: OpenAI, xAI, DashScope) — no tool caching;
            //   always send full tool definitions in the `tools` field.
            //
            // Local inference (llama.cpp, LM Studio, Ollama) — KV cache prefix
            //   stability is critical. Embed tools in system prompt as a
            //   deterministic text block (TCSP) so they become part of the
            //   stable KV-cached prefix. Omit the `tools` field.
            let (skip_tools, tools_in_system_prompt, per_entry_tools, per_entry_system) =
                if entry.provider_kind == ProviderKind::Anthropic {
                    ...
                    // Anthropic: use `tools` field with cache_control
                    let tools_val = (!tools.is_empty()).then(|| tools.clone());
                    (sk, false, tools_val, system.clone())
                } else if is_local {
                    // Local inference: tools in system prompt, no `tools` field
                    ...
                    // Cloud OpenAI-compat / xAI / DashScope: always send tools inline
                    let tools_val = (!tools.is_empty()).then(|| tools.clone());
            ...
            match attempt {
                Ok(events) => {
                    // Persist tools_hash so future requests can skip tools.
