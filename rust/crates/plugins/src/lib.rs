...
pub struct PluginManifest {
    ...
    // TODO(phase-2): wire manifest.commands into CommandRegistry.
    // Phase 1 (2026-06-04, spec §5.5): the CommandHandler trait + CommandRegistry
    // are introduced in `commands` crate, but the plugin loader does not yet
    // register plugin-declared commands. The manifest's `commands` field is
    // still read (so plugin validation and reporting see the entries), but it
    // is not dispatched at runtime — built-in slash dispatch is unchanged.
    // Phase 2 trigger criterion: see spec §11 Open Questions.
    #[serde(default)]
...
impl PluginDefinition {
    pub fn mcp_servers(&self) -> Option<&Value> {
        match self {
            Self::Builtin(p) => p.mcp_servers.as_ref(),
            Self::Bundled(p) => p.mcp_servers.as_ref(),
    ...
    pub fn agent_paths(&self) -> &[PathBuf] {
        match self {
            Self::Builtin(p) => &p.agents,
            Self::Bundled(p) => &p.agents,
    ...
    pub fn skill_paths(&self) -> &[PathBuf] {
        match self {
            Self::Builtin(p) => &p.skills,
            Self::Bundled(p) => &p.skills,
...
impl PluginManager {
    ...
    pub fn plugin_registry_report(&self) -> Result<PluginRegistryReport, PluginError> {
        ...
        for root in &self.config.plugin_roots {
            ...
            match load_plugin_definition(root, PluginKind::External, source, EXTERNAL_MARKETPLACE)
            {
                Ok(plugin) => {
...
pub fn builtin_plugins() -> Vec<PluginDefinition> {
    vec![PluginDefinition::Builtin(BuiltinPlugin {
        ...
        lifecycle: PluginLifecycleSpec::default(),
        tools: Vec::new(),
        mcp_servers: None,
        agents: Vec::new(),
        skills: Vec::new(),
