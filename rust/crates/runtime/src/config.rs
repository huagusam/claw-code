...
pub use claw_code_plugin_types::RuntimePluginConfig;
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        ...
        let canonical_home = user_home_dir().and_then(|h| h.canonicalize().ok());
        ...
        for ancestor in cwd_ancestors.iter().rev() {
            ...
            let project_paths = [
                ...
                (ConfigSource::Local, ancestor.join(".claw").join("settings.local.json")),
                (ConfigSource::Project, ancestor.join(".claude").join("settings.json")),
                (ConfigSource::Local, ancestor.join(".claude").join("settings.local.json")),
            ];
...
mod tests {
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        // Windows. The test is hermetic: 3 entries, exactly the 3 fixtures.
