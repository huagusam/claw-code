...
For large .txt and .md files exceeding 20,000 bytes, analyze content internally and return only explicitly requested information.",
        ...
        ToolSpec {
            name: "new_file",
            description: "Create a new file. If the target file already exists, enable the `force` flag to fully overwrite it. Use `edit_file` for partial changes to existing files.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
...
If `new_string` is shorter than `old_string`, confirm you are intentionally removing content.",
        ...
        ToolSpec {
            name: "undo",
            description: "Undo a prior `edit_file` operation by reading its diff file and applying the inverse string replacement. \
Provide the `diff_path` from the previous `edit_file` result's `diffPath` field. \
The diff file is automatically deleted after a successful undo. \
Multiple undos on the same diff file will fail because the diff file is removed after the first undo.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "diff_path": {
        ...
        ToolSpec {
            name: "WebFind",
            description:
                "Fetch a URL and return only the lines that match a substring pattern, with line number, column, and trimmed context. Use this when you need a specific value (version, token, error code, identifier) from a page without paying the cost of the full body. Preferred over WebFetch when the answer is a string the page already contains.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "format": "uri" },
        ...
        ToolSpec {
            name: "WebSearch",
            description: "Search the web for current information. Use region=\"cn\" for China (cn.bing.com), region=\"international\" or涓嶅～ for international (www.bing.com).",
            input_schema: json!({
                "type": "object",
                "properties": {
                    ...
                    "region": { 
                        ...
                        "description": "鎼滅储鍖哄煙: cn=鍥藉唴鐗? international=鍥介檯鐗?榛樿)"
                    },
                    "allowed_domains": {
                        "type": "array",
                        "items": { "type": "string" }
        ...
        ToolSpec {
            name: "Skill",
            description: "Load a local skill definition and its instructions. When the user references a skill with $name (e.g. $frontend-ui-engineering), call this tool to load that skill's full instructions from SKILL.md. Use ListSkills to discover available skills. The 'skill' field should be the skill name (with or without the $ prefix). The 'args' field passes optional arguments to the skill.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            name: "Agent",
            description: "Launch a specialized agent task and persist its handoff metadata. When the user references an agent with @name, call this tool to delegate work to that agent. Use ListAgents to discover available agents. The 'name' field should match the agent's @reference name. The 'subagent_type' field selects the agent's tool set: 'general-purpose' (full tools), 'Explore' (read-only search), 'Plan' (read + TodoWrite), or 'Verification' (bash + read). The 'model' field overrides the default agent model. Agent runs asynchronously; use AgentGet to check status.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "description": { "type": "string" },
        ...
        ToolSpec {
            name: "AgentGet",
            description: "Query an agent's status without loading its full output. Returns lightweight status (running/completed/failed), error message if any, and file paths. Use Read tool on `outputFile` if you need the complete content. This saves tokens when you only need to check if an agent finished.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "agentId": { "type": "string" }
        ...
        ToolSpec {
            name: "PowerShell",
            description: "Execute a PowerShell command with optional timeout. The runtime prepends a UTF-8 encoding preamble and prefers `pwsh` (PowerShell 7) when available, so non-ASCII file paths and content round-trip correctly. Always pass file paths via the `-LiteralPath` parameter to cmdlets (Get-Content, Get-Item, etc.) so brackets and other special characters are not interpreted as wildcards.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "command": { "type": "string" },
