...
fn now_secs() -> u64 {
    SystemTime::now()
        ...
        .unwrap_or_else(|e| {
            eprintln!("[worker_boot] system clock is before epoch ({e}); using 0");
            Duration::ZERO
        })
        .as_secs()
}
...
fn emit_state_file(worker: &Worker) {
    ...
    if let Err(e) = std::fs::create_dir_all(&state_dir) {
        eprintln!("[worker] failed to create state dir for {}: {e}", worker.worker_id);
        return;
    }
    ...
    match serde_json::to_string_pretty(&snapshot) {
        Ok(json) => {
            if let Err(e) = std::fs::write(&tmp_path, json) {
                eprintln!("[worker] failed to write state file for {}: {e}", worker.worker_id);
                return;
            }
            if let Err(e) = std::fs::rename(&tmp_path, &state_path) {
                eprintln!("[worker] failed to rename state file for {}: {e}", worker.worker_id);
            }
        }
        Err(e) => {
            eprintln!("[worker] failed to serialize state for {}: {e}", worker.worker_id);
        }
    }
}
...
fn normalize_path(path: &str) -> PathBuf {
    let result = crate::boundary::canonicalize_maybe_missing(Path::new(path));
    #[cfg(windows)]
    {
...
fn cwd_matches_observed_target(expected_cwd: &str, observed_cwd: &str) -> bool {
    ...
    // Match on base directory name (most reliable), OR full suffix if the
    // observed path is a proper suffix of expected (matching on component
    // boundaries rather than raw ends_with to avoid false positives like
    // "repo-backup" matching "repo").
    expected_base == observed_base
        || expected
            .to_string_lossy()
            .strip_suffix(observed_cwd)
            .is_some_and(|prefix| prefix.ends_with('/') || prefix.ends_with('\\'))
        || observed_cwd
            .strip_suffix(expected.to_string_lossy().as_ref())
            .is_some_and(|prefix| prefix.ends_with('/') || prefix.ends_with('\\'))
}
...
mod tests {
    ...
    fn temp_cwd(name: &str) -> String {
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir()
            .join(format!("claw-wb-{name}-{nanos}"))
            .to_str()
            .expect("utf-8")
            .to_string()
    }
    ...
    fn allowlisted_trust_prompt_auto_resolves_then_reaches_ready_state() {
        ...
        let worker = registry.create(&cwd, &[parent], true);
    ...
    fn startup_timeout_classifies_tool_permission_prompt() {
        let cwd = temp_cwd("repo-mcp-timeout");
        let registry = WorkerRegistry::new();
        let worker = registry.create(&cwd, &[], true);
    ...
    fn prompt_delivery_detects_wrong_target_and_replays_to_expected_worker() {
        let cwd = temp_cwd("repo-target-a");
        let wrong_cwd = temp_cwd("repo-target-b");
        let registry = WorkerRegistry::new();
        let worker = registry.create(&cwd, &[], true);
        registry
            .observe(&worker.worker_id, "Ready for input\n>")
        ...
        let recovered = registry
            .observe(
                &worker.worker_id,
                &format!("{wrong_cwd} % Run the worker bootstrap tests\nzsh: command not found: Run"),
            )
        ...
        assert_eq!(
            ...
            Some(WorkerEventPayload::PromptDelivery {
                ...
                observed_cwd: Some(wrong_cwd.clone()),
                observed_prompt_preview: None,
                task_receipt: None,
                recovery_armed: false,
            })
    ...
    fn await_ready_surfaces_blocked_or_ready_worker_state() {
        let cwd = temp_cwd("repo-d");
        let registry = WorkerRegistry::new();
        let worker = registry.create(&cwd, &[], false);
