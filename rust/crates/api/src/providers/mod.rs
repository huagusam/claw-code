...
/// (llama.cpp, LM Studio, Ollama, vLLM, etc.) where KV cache prefix
/// stability is critical and tools-in-system-prompt is beneficial.
///
/// Detection heuristics:
/// 1. Explicit opt-in via `CLAW_LOCAL_INFERENCE=true`
/// 2. `OPENAI_BASE_URL` points to localhost/127.0.0.1
/// 3. `OPENAI_BASE_URL` is set without `OPENAI_API_KEY` (no-auth local server)
#[must_use]
pub fn is_local_inference() -> bool {
    ...
    if local_hosts.iter().any(|h| base_url.contains(h)) {
        return true;
    }
    // OPENAI_BASE_URL set without API key = likely a no-auth local server
