...
use plugins::{PluginCommand, PluginHooks, PluginManager, PluginManagerConfig, PluginRegistry};
...
fn detect_mentioned_agent(
    ...
    let content = if agents.iter().any(|a| a.name() == name) {
        ...
        find_agent_file(&cwd, &name).and_then(|p| std::fs::read_to_string(p).ok())
    };
...
impl LiveCli {
    ...
    fn resolve_plugin_command(&self, name: &str) -> Option<PluginCommand> {
        let mut by_name: std::collections::HashMap<String, PluginCommand> = std::collections::HashMap::new();
        for command in self.runtime.plugin_registry.aggregated_commands() {
            by_name
...
"#,
        ...
        let normalized_cwd =
            context.cwd.canonicalize().unwrap_or_else(|_| context.cwd.clone());
