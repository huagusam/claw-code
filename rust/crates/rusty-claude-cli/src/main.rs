...
use runtime::{
    ...
    extract_embedded_tools, ApiClient, ApiRequest, AssistantEvent, CompactionConfig, ConfigLoader, ConfigSource,
...
use tools::{
    execute_tool, GlobalToolRegistry, RuntimeToolDefinition, ToolSearchOutput,
};
...
fn read_piped_stdin() -> Option<String> {
    ...
    if let Err(e) = io::stdin().read_to_string(&mut buffer) {
        ...
        eprintln!("[stdin] partial read ({buf_len} bytes) before error: {e}", buf_len = buffer.len());
    }
...
fn agent_invocation_hint(name: &str, agent: &commands::AgentSummary) -> String {
    ...
    hint.push_str("⚠️ 你刚刚加载了一个 agent 的配置文件。这表示用户希望你将此任务委派给该 agent 处理。\n\n");
    hint.push_str("⚠️ 你现在必须调用 Agent 工具来委派此任务，不要自己直接执行。\n\n");
    hint.push_str("请立即调用 Agent 工具，参数如下：\n");
    hint.push_str(&format!("  <name>{} </name>\n", name));
    hint.push_str("  <description>用户的任务摘要</description>\n");
    hint.push_str("  <prompt>用户的完整请求</prompt>\n");
        ...
        if let Some(agent) = agents.iter().find(|a| a.name() == mention) {
            let expansion = if let Some(agent_path) = find_agent_file(&cwd, &mention) {
                match std::fs::read_to_string(&agent_path) {
                    ...
                    Err(e) => {
                        eprintln!("[mentions] failed to read @{} file at {}: {e}", mention, agent_path.display());
...
fn find_agent_file(cwd: &Path, name: &str) -> Option<PathBuf> {
    for ancestor in cwd.ancestors() {
        for config_dir in &[".claw", ".claude"] {
            let dir = ancestor.join(config_dir).join("agents");
            if !dir.is_dir() {
                continue;
            }
            // Only search for name.md, name.txt, name.toml
...
/// Generic file finder for agents/skills in specified subdirectories.
fn find_mention_file_in_subdir(cwd: &Path, name: &str, subdirs: &[&str]) -> Option<PathBuf> {
    for ancestor in cwd.ancestors() {
        // .claw/ takes priority (project-native), then .claude/ (compat)
        for config_dir in &[".claw", ".claude"] {
            for subdir in subdirs {
                let dir = ancestor.join(config_dir).join(subdir);
                if !dir.is_dir() {
                    continue;
                }
...
fn run_repl(
    ...
    let mut editor =
        input::LineEditor::new("> ", cli.repl_completion_candidates().unwrap_or_default(), mention_names, skill_names);
    ...
    loop {
        ...
        match editor.read_line_interactive()? {
            input::ReadOutcome::Submit(input) => {
                ...
                let image_dir = cli.runtime.session().workspace_root.as_ref().map(|root| root.join(".claw").join("images"));
                let mut resolved_input = input::resolve_drag_drop_files(&trimmed, image_dir.as_deref());
...
fn build_plugin_mcp_servers(
    ...
    for (server_name, (_plugin_name, value)) in plugin_registry.mcp_server_configs() {
        ...
        match runtime::parse_mcp_server_config(
            ...
            Err(error) => {
                eprintln!(
                    "[plugin mcp] skipped {server_name}: {error}",
                    error = error
                );
            }
        }
...
fn build_plugin_agents(
    plugin_registry: &PluginRegistry,
) -> Vec<commands::AgentSummary> {
...
fn render_brand_text() -> String {
    ...
    for row in 0..BANNER_ICON_LINES.len() {
        ...
        out.push_str("  ");
        if row == 0 {
            // Row 0: "claw-code" in bold + per-char gradient, then plain version tail.
            out.push_str(&gradient_bold("claw-code", (255, 90, 30), (255, 230, 110)));
            out.push_str(" v0.1.0-local");
        } else {
...
impl LiveCli {
    ...
    fn mention_candidates(&self) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        // Extract agent names from plugin registry
        let agents = build_plugin_agents(&self.runtime.plugin_registry);
        for agent in &agents {
            names.push(agent.name().to_string());
        }
        // Also scan .claude/agents/ dirs for file-based agent definitions
        let cwd = std::env::current_dir().unwrap_or_default();
        for ancestor in cwd.ancestors() {
            let agents_dir = ancestor.join(".claude").join("agents");
            if agents_dir.is_dir() {
                if let Ok(entries) = std::fs::read_dir(&agents_dir) {
                    for entry in entries.flatten() {
                        if let Some(name) = entry.file_name().to_str() {
                            if let Some(stem) = name.rsplit('.').last() {
                                if !names.contains(&stem.to_string()) {
                                    names.push(stem.to_string());
                                }
                            } else if !names.contains(&name.to_string()) {
    ...
    fn skill_candidates(&self) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        let cwd = std::env::current_dir().unwrap_or_default();
        for ancestor in cwd.ancestors() {
            for dir_name in &[".claw", ".claude", ".omc", ".agents", ".codex"] {
                let skills_dir = ancestor.join(dir_name).join("skills");
                if skills_dir.is_dir() {
                    if let Ok(entries) = std::fs::read_dir(&skills_dir) {
                        for entry in entries.flatten() {
                            if let Some(name) = entry.file_name().to_str() {
        ...
        match result {
            Ok(summary) => {
                ...
                if let Some(event) = summary.auto_compaction {
                    println!(
                        "{}",
                        format_auto_compaction_notice(event.removed_message_count, event.savings_ratio)
                    );
                }
...
fn confirm_session_deletion(session_id: &str) -> bool {
    if let Err(e) = write!(io::stdout(), "Delete session '{session_id}'? This cannot be undone. [y/N]: ") {
...
fn render_last_tool_debug_report(session: &Session) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let mut lines = vec![
        ...
        "  Input".to_string(),
        indent_block(&last_tool_use.2.to_string(), 4),
    ];
...
fn build_runtime_plugin_state_with_loader(
    ...
    let (mcp_state, runtime_tools) =
        build_runtime_mcp_state(runtime_config, &plugin_mcp_servers)?;
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let (messages, cached_values) = {
            ...
            if let Some(cache) = &self.message_cache {
                if cache.last_ptr == msg_ptr && cache.last_len <= msg_len {
                    ...
                    (Arc::clone(&cache.input_messages), Arc::clone(&cache.cached_values))
                } else {
        ...
        let message_request = MessageRequest {
            ...
            system: (!request.system_prompt.is_empty())
                .then(|| Arc::clone(&request.system_prompt)),
            tools: self
                .enable_tools
                ...
                ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                    ...
                    ContentBlockDelta::ThinkingDelta { thinking } => {
                        if !thinking.is_empty() {
                            if !block_has_thinking_summary {
                                write!(out, "\x1b[38;5;244m\x1b[2m\x1b[3m▶ Thinking [")
                                    .and_then(|()| out.flush())
                                    .map_err(|error| RuntimeError::new(error.to_string()))?;
                            ...
                            let full_clean = strip_tool_xml_markup(&accumulated_thinking);
                            let clean_display = full_clean.trim_end_matches('\n').to_string();
    ...
    while let Some(tag_start) = remaining.find('<') {
        ...
        if let Some(close_name) = after.find('>') {
            ...
            let close_tag: Option<&str> = if tag_content.eq_ignore_ascii_case("tool_call") {
                Some("</tool_call>")
            } else if tag_content.starts_with("function=") || tag_content.eq_ignore_ascii_case("function") {
                Some("</function>")
...
fn format_tool_call_start(name: &str, raw_input: &str) -> String {
    ...
    let detail = match name {
        ...
        "read_file" | "Read" => {
            ...
            let display_path = if cfg!(windows) && path.starts_with('\\') {
                // 处理 \\?\ 前缀的Windows路径
    ...
    match block {
        ...
        OutputContentBlock::Thinking { thinking, .. } => {
            if streaming_tool_input && thinking.is_empty() {
                ...
                // Extract embedded tool calls from thinking XML and push as real ToolUse events.
                ...
                if !clean.trim().is_empty() {
                    let width = crossterm::terminal::size()
                        ...
                        .unwrap_or(80);
                    let rendered = TerminalRenderer::new().render_reasoning_block(&clean, width, false);
                    write!(out, "{rendered}")
                        .and_then(|()| out.flush())
                        .map_err(|error| RuntimeError::new(error.to_string()))?;
                    events.push(AssistantEvent::Thinking(clean));
                    ...
                    let width = crossterm::terminal::size()
                        .map(|(cols, _)| cols as usize)
                        .unwrap_or(80);
                    let rendered = TerminalRenderer::new().render_reasoning_block(&thinking, width, false);
                    write!(out, "{rendered}")
                        .and_then(|()| out.flush())
                        .map_err(|error| RuntimeError::new(error.to_string()))?;
...
mod tests {
    ...
    use super::{
        ...
        describe_tool_progress, filter_tool_specs,
        ...
        format_resume_report, format_status_report, format_tool_call_start,
        format_tool_result, format_unknown_slash_command,
...
UU conflicted.rs",
    ...
    fn response_to_events_renders_collapsed_thinking_summary() {
        ...
        let rendered = String::from_utf8(out).expect("utf8");
        assert!(rendered.contains("\x1b[38;5;244m\x1b[2m\x1b[3m▶ Thinking [step 1]\x1b[0m\n"), "thinking text should be rendered gray+italic: {rendered:?}");
