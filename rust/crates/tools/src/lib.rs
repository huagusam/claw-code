...
fn resolve_skill_path_in_skills_dir(
    ...
    for entry in entries.flatten() {
        ...
        if dir_name.eq_ignore_ascii_case(requested)
            ...
            || resolved
                .file_stem()
                .map_or(false, |s| s.to_string_lossy().eq_ignore_ascii_case(requested))
        {
    ...
    F: FnOnce(agents::AgentJob) -> Result<agents::AgentHandle, String>,
{
    ...
    let job = agents::AgentJob {
        ...
        prompt: input.prompt,
        system_prompt: {
            // Generic "you are a background sub-agent" guardrails (don't ask the
            // user, return results, work only on the delegated task). When the
            // caller supplies a persona (`system_prompt`), keep those guardrails
            // and append the persona — replacing it outright would drop the
            // contract that makes delegated agents safe to run unattended.
